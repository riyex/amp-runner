...
Complete. README and architecture documentation now describe verified update behavior, the plan is checked through Task 9, and a hosted fake-Amp integration test covers the complete update/restart flow without reading or modifying `/Users/rgibbons/.amp/bin/amp` or real application preferences.
...
- `ARCHITECTURE.md`: documents the single controller/schedule, standardized path keys, generic launch-version provider, coordinator-only joining/policy, porcelain migration boundary, bounded state, separate update/runner health, and persisted non-secret update preferences/notification ledgers.
...
`RunnerUpdateOrchestrationTests.testFakeAmpEndToEndCheckInstallNotificationAndIdleRestart` creates an executable shell fixture and version/call-ledger files under a unique temporary directory. The production bounded command executor invokes that fixture directly through an injected `AmpUpdateController`; release fetching, profile state, restart recording, notifications, and preferences use test-only injected dependencies/suites. Cleanup is deferred for the temporary fixture directory.

The fixture proves: one release request → `1.0.0` update available → one `update --porcelain` for the shared standardized path → installed `2.0.0` and exactly one aggregate restart-required notification → no restart while Working → one restart after Online/idle → running `2.0.0` and Up to Date. It never resolves or invokes the real Amp path.
...
- Unregistration not pruning every private controller cache: bounded by distinct executable paths/releases and not polling; no full-verification or low-memory blocker.
...
- Supervisor fixture cleanup: the new integrated fixture uses deferred temporary-directory cleanup; existing hosted fixtures still rely on explicit test cleanup, with no observed leak/failure.
...
- System notification authorization coverage and suite-scoped defaults cleanup: hosted builders and injected defaults cover policy; real-center integration remains intentionally outside safe verification.
